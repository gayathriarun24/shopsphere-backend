  ...
  try {
    ...
    const formattedOrderItems = await Promise.all(
      orderItems.map(async (item) => {
        ...
        const productTitle = 
          ...
          item.title || 
          item.name || 
          (typeof item.product === 'object' && item.product !== null ? (item.product.title || item.product.name) : null) || 
          'Product Item';

        const productPrice = item.price !== undefined ? item.price : (productDoc ? productDoc.price : 0);
    ...
    try {
      ...
      await brevo.transactionalEmails.sendTransacEmail({
        ...
        htmlContent: '<div style="font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Helvetica, Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 40px 20px; background-color: #ffffff;">' +
          ...
          '<p style="font-size: 14px; color: #555; line-height: 1.5; margin-bottom: 24px;">We\'re getting your order ready to be shipped. We will notify you when it has been sent.</p>' +
          ...
          '<table width="100%" cellpadding="0" cellspacing="0" style="font-size: 14px; color: #555; margin-bottom: 30px;">' +
            '<tr><td style="padding: 6px 0;" align="right">Subtotal</td><td style="padding: 6px 0; width: 120px;" align="right">Rs. ' + createdOrder.totalAmount.toFixed(2) + '</td></tr>' +
            '<tr><td style="padding: 6px 0;" align="right">Shipping</td><td style="padding: 6px 0;" align="right">Rs. 0.00</td></tr>' +
            '<tr><td style="padding: 6px 0; border-bottom: 1px solid #e5e7eb; padding-bottom: 12px;" align="right">Taxes</td><td style="padding: 6px 0; border-bottom: 1px solid #e5e7eb; padding-bottom: 12px;" align="right">Rs. 0.00</td></tr>' +
            '<tr><td style="padding: 15px 0; font-size: 16px; font-weight: 600; color: #111;" align="right">Total</td><td style="padding: 15px 0; font-size: 18px; font-weight: 700; color: #111;" align="right">Rs. ' + createdOrder.totalAmount.toFixed(2) + '</td></tr>' +
          '</table>' +
          '<div style="text-align: center; font-size: 12px; color: #9ca3af; border-top: 1px solid #e5e7eb; padding-top: 20px; margin-top: 40px;">&copy; ' + new Date().getFullYear() + ' ShopSphere. All rights reserved.</div>' +
        '</div>'
      });
